...
The output will look something like this:
    ...
    [*] osqueryd version 3.2.6
    [*] osqueryi version 3.2.6
    [*] go version go1.11.5 linux/amd64
    [*] Mage Build Tool v1.8.0-6-g5dee1ad
...
* The **CCPF_PROJECT_HOME** directory's Makefile feeds Jsonnet a project.ccpf-defn.jsonnet and generates all necessary
  assets / artifacts (e.g. SSG configs, Dockerfile, docker-compose.yml, etc.).

## Scripts

* **CCPF_HOME/bin/ccpf-init** is a convenience script to symlink the Makefile in a **CCPF_PROJECT_HOME**.
...
This allows each project to have it's own Makefile but conveniently
links to a master Makefile unless the project has something special.

If the CCPF_PROJECT_HOME does not contain a Makefile, then the **ccpf-make** convenience script may be used as a replacement.
...
* **unconfigure** cleans up generated files, etc. (**DESTRUCTIVE**)
...
* **etc**. Configuration files for binaries and scripts that are used by the
  CCPF utilities and Makefiles may, optionally, reside in this directory.
* **hugo-publish-main**. This directory contains the final, static content, that is 
  ...
  build).  This directory is called _hugo-spublish-main_ because there may be more 
  than one. The *-_main_ would be replaced with *-_identity_.
* **hugo-src-main**. The directory in which the content which an SSG (e.g. Hugo) 
  ...
  directory is called _hugo-src-main_ because there may be more than one. The
  *-_main_ would be replaced with *-_identity_.
* **support**. CCPF documentation and other support collateral may, optionally,
  reside in this directory.
* **tasks**. Custom Makefiles, Mage scripts, and other task runner code may,
  optionally, reside in this directory.
